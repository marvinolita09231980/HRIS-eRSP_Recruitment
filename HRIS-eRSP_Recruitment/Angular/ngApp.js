var ng_eRSP_App = angular.module('ng_eRSP_App', ['ngFileUpload'])

ng_eRSP_App.run(function () {
  ...
  //this fucntion is called after refreshTable to return to the current dataTable page
   ...
   // String.prototype
    ...
    String.prototype.replaceClass = function (cls1, cls2) {
        var id = this
        $("#" + id).removeClass(cls1)
        $("#" + id).addClass(cls2)
    }
    String.prototype.addSpinner = function (cls) {
        var id = this
        $("#" + id).removeClass(cls)
        $("#" + id).addClass("fa-spinner fa-spin")
    }
    String.prototype.removeSpinner = function (cls) {
        var id = this
    ...
    String.prototype.hasClass = function (cls) {
        var id = this
        var b = $("#" + id).hasClass(cls)
        if (b) return true
        else return false
    }
    String.prototype.disabled = function () {
        var id = this
    ...
    Array.prototype.in = function (data,compare) {
        ...
        for (x = 0; x < data_len; x++) {
            var dt = list.select(data[x].value, compare)
            retdata.push(dt[0])
        }
        return retdata;
    }
    Array.prototype.checkMultipleCB = function (id,compare,value) {
        var this_len = this.length
        var list = this
       
        for (x = 0; x < this_len; x++) {
            
            if (list[x][compare] == value) {
    ...
    Array.prototype.populateFields3 = function (obj, row) {
        var data = obj
        var thisData = this[row]
        var i_key = Object.keys(obj)
        var f_key = Object.keys(thisData)
    ...
    Array.prototype.Nselect = function (code, prop) {
        return this.filter(function (d) {
            return d[prop] != code
        })
    }
...
ng_eRSP_App.service("commonScript", ["$compile", "$filter", function (c, f) {
    ...
    createSingleItemArray: function (data, prop) {
        var array = []
        var data_len = data.length
        for (x = 0; x < data_len; x++) {
            array.push({value:data[x][prop]})
        }
        return array
     },
    ...
    Validate1Field2: function (id,wn) {
        var val = $("#" + id).val();
        if (this.elEmpty(val)) {
            this.required2(id, wn)
            return false;
        }
        else {
            this.notrequired2(id);
    ...
    array_to_str: function(data, prop) {
        var str = ""
        var len = data.length
        for (x = 0; x < len; x++) {
        ...
        uncheck_box_multiple : function (id, data) {
            var len = data.length
            for (x = 0; x < len; x++) {
                if ($("#" + id).is(':checked')) {
                    $("#" + id).is(':checked') = false
    ...
    faIconLoaderShow : function(id, icon) {
        $("#" + id + " .fa").removeClass(icon);
        $("#" + id + " .fa").addClass("fa-spinner fa-spin");
        $("." + id).prop("disabled", true);
    },

    faIconLoaderHide: function (id, icon) {
        $("#" + id + " .fa").removeClass("fa-spinner fa-spin");
        $("#" + id + " .fa").addClass(icon);
        $("#" + id).prop("disabled", false);
    },
    ...
    var re = /\/Date\(([0-9]*)\)\//;
        ...
        return function (x) {
            ...
            if (m) {
                ...
                mo = dt.getMonth();
                dy = dt.getDay();
                return yr + "-" + leadingZeroMonth(mo) + "-" +leadingZeroMonth(dy);
            }
            else {
                return null;
            }
...
ng_eRSP_App.filter("year", function () {
    var re = /\/Date\(([0-9]*)\)\//;
    return function (x) {
        var m = x.match(re);
        if (m) {
...
ng_eRSP_App.directive("headerSearch", ["commonScript", function (cs) {
    return {
        restrict: 'C',
...
ng_eRSP_App.directive("setTableRow", ["commonScript", function (cs) {
    return {
        restrict: 'C',
        link: function (scope, elem, attrs) {
...
ng_eRSP_App.directive('removeReq', ["commonScript", function (cs) {
    //************************************// 
...
ng_eRSP_App.directive('removeReqDate', ["commonScript", function (cs) {
    //************************************// 
    //*** this directive remove the required warning of the specified input field on keyup
    //************************************// 
    return {
...
ng_eRSP_App.directive('removeReqOption', ["commonScript", function (cs) {
    //************************************// 
...
ng_eRSP_App.directive('dateMinVal', ["commonScript", function (cs) {
...
ng_eRSP_App.directive('dateMaxVal', ["commonScript", function (cs) {
...
ng_eRSP_App.directive('selectEditable', ["commonScript", function (cs) {
    ...
    return {
        ...
        link: function (scope, elem, attrs) {
            elem.on('click', function () {
                ...
                var top = pos.top
                var left = pos.left
                var parent = document.getElementsByClassName('selectEditable')
                var divTag = document.createElement("div");
                divTag.className = "no-padding";
                divTag.innerHTML = "<input type='text' class='form-control'/>";
                parent.appendChild(divTag)
                var el2 = cs.D_cE("Input")
...
ng_eRSP_App.directive('ngSelect2', ["commonScript", function (cs) {
...
ng_eRSP_App.directive('norequired', ["commonScript", function (cs) {
    var t = "type"
    var hA = "hasAttribute"
    //************************************// 
